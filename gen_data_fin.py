...
def write_instance_to_example_files(instances, max_seq_length,
                                    ...
                                    output_files, global_seq_length=0):
    ...
    for (inst_index, instance) in enumerate(instances):
        ...
        if global_seq_length > 0:
            global_tokens = ['[GLOBAL_{}'.format(x) for x in range(1, global_seq_length + 1)]
            try:
                global_input_ids = vocab.convert_tokens_to_ids(global_tokens)
            except:
                print(global_tokens)
        ...
        if global_seq_length > 0:
            features["global_input_ids"] = create_int_feature(global_input_ids)
...
def gen_samples(data,
    ...
    write_instance_to_example_files(instances, max_seq_length,
                                    max_predictions_per_seq, vocab,
                                    [output_filename], global_seq_length)
