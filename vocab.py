...
class FreqVocab(object):
    ...
    def __init__(self, user_to_list):
        ...
        self.global_tokens = ['[GLOBAL_{}'.format(x) for x in range(1, 257)]
...
#             self.token_to_ids[user] = len(self.token_to_ids) + 1
    ...
    def get_global_token_count(self):
        return len(self.global_tokens)

    def get_global_token(self):
        return self.global_tokens

    def get_vocab_size(self):
        return self.get_item_count() + self.get_special_token_count() + self.get_global_token_count() + 1 #self.get_user_count()
